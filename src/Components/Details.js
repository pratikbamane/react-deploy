...
class Details extends React.Component {
    ...
    render() {
        const { restaurant, galleryModalIsOpen, orderModalIsOpen, menuItems, subTotal, formModalIsOpen, userName, contactNumber, address, email } = this.state;
        return (
            <div>
                <div>
                    <img src={`../${restaurant.image}`} width="100%" height="500px" />
                    <button className="gallery-button" onClick={() => this.handleClick('galleryModalIsOpen', true)}>Click to see Image Gallery</button>
                </div>

                <div className="heading1">{restaurant.name}</div>
                <button className="btn btn-danger" style={{ float: 'right', margin: '15px', backgroundColor: '#ce0505' }} 
                onClick={() => this.handleClick('orderModalIsOpen', true)}>Place Online Order</button>
                ...
                <Modal
                  ...
                  >
                      <div>
                          ...
                          <h3 className="sub-total">SubTotal : {subTotal}</h3>
